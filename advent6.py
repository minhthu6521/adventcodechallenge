...
def get_group_distinct_answer(all):
    total = 0
    for g in all:
        answers = get_distinct_answer(g)
        total += len(answers)
    return total


def get_same_answer(all):
    result = []
    for a in all:
        result += [c for c in a]
    return set([r for r in result if result.count(r) == len(all)])


def get_group_same_answer(all):
    total = 0
    for g in all:
        answers = get_same_answer(g)
        total += len(answers)
    return total


if __name__ == '__main__':
    list_of_answers_by_group = INPUT.split("\n\n")
    separated_answer = [a.split("\n") for a in list_of_answers_by_group]
    print(get_group_same_answer(separated_answer))