INPUT = """dull blue bags contain 2 dotted green bags, 1 dull brown bag, 3 striped tomato bags, 5 muted blue bags.
...
def direct_contain(bag, rules, check):
    inner = rules[bag]
    if not inner.keys():
        return
    if check in inner.keys():
        return bag
    else:
        for ibag in inner.keys():
            contain = direct_contain(ibag, rules, check)
            if contain:
                return bag


def inner_content_count(bag, rules):
    total = 0
    for ibag in rules[bag].keys():
        total += rules[bag][ibag]
        if rules[ibag].keys():
            total += inner_content_count(ibag, rules) * rules[bag][ibag]
    return total


def can_contain(rules, all_color, check):
    count = 0
    for bag in all_color:
        if bag != check:
            possible = direct_contain(bag, rules, check)
            if possible:
                count += 1
    return count
...
if __name__ == '__main__':
    rules = INPUT.replace(".", "").split("\n")
    outer_inner = {}
    all_bags = []
    for rule in rules:
        [outer_bag, inner_content] = rule.split(" contain ")
        outer_bag_name = outer_bag.replace("bags", "").strip()
        all_bags.append(outer_bag_name)
        outer_inner[outer_bag_name] = {}
        if inner_content == "no other bags":
            continue
        else:
            inner_content = inner_content.split(",")
            for icontent in inner_content:
                icontent = icontent.strip()
                quantity = icontent[0]
                bag_name = icontent[1:].replace("bags", "").replace("bag", "").strip()
                all_bags.append(bag_name)
                outer_inner[outer_bag_name][bag_name] = int(quantity)
    all_bags = set(all_bags)
    #print(can_contain(outer_inner, all_bags, "shiny gold"))
    print(inner_content_count("shiny gold", outer_inner))