...
def is_valid(details, optional=["cid"]):
    required_test = required()
    details_s = details.replace("\n", " ")
    details_s = details_s.split(" ")
    detail_d = {}
    for d in details_s:
        [key, value] = d.split(":")
        detail_d[key] = value

    for r in required_test.keys():
        if (r) not in detail_d.keys() and r not in optional:
            return False
        if not required_test[r](detail_d.get(r)):
            print(r, detail_d.get(r))
            return False
    return True


def check_batch_is_valid(input=INPUT):
    details = input.split("\n\n")
    count = 0
    for d in details:
        if is_valid(d):
