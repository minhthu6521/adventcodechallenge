...
MESSAGES = """bbbbbbbbbaaaabbaababbabbaaabbbabbbbaaabb
...
def check_straight(message, rule_content):
    if not message:
        return None
    char = rule_content.replace('"', "")
    return message[1:] if char == message[0] else None


def check_sub(message, rule_content):
    sub_rules_group = rule_content.split(" | ")
    messages = []
    for rule in sub_rules_group:
        messages.append(check_valid_message(message, rule, rules))
    return next((m for m in messages if m is not None), None)


def check_valid_message(message, rule_content, rules):
    if '"' in rule_content:
        return check_straight(message, rule_content)
    elif "|" in rule_content:
        return check_sub(message, rule_content)
    else:
        sub_rules = rule_content.split(" ")
        for index, rule_num in enumerate(sub_rules):
            message = check_valid_message(message, rules[rule_num], rules)
            if message is None:
                return None
        return message


def execute(messages, rules, rule_start):
    count = 0
    for message in messages:
        if check_valid_message(message, rules[rule_start], rules) == "":
            count += 1
    return count


def construct_regex(rule_content, rules):
    if '"' in rule_content:
        char = rule_content.replace('"', "")
        return char
    elif "|" in rule_content:
        ...
        or_group = []
        for rule in sub_rules_group:
            subregex = construct_regex(rule, rules)
            or_group.append(subregex)
        return "(" + "|".join(or_group) + ")"
    else:
        regex = ""
        sub_rules = rule_content.split(" ")
        for index, rule_num in enumerate(sub_rules):
            if rule_num == "8":
                sub = "(" + construct_regex(rules["42"], rules) + ")+"
            elif rule_num == "11":
                subm = "(" + construct_regex(rules["42"], rules) + "{num}" + construct_regex(rules["31"], rules) + "{num})"
                sub = []
                for i in range(1, 10):
                    sub.append(subm.format(num="{" + str(i) + "}"))
                sub = "|".join(sub)
            else:
                sub = construct_regex(rules[rule_num], rules)
            regex += "(" + sub + ")"
        return regex


def execute_with_regex(messages, rules):
    count = 0
    regex = construct_regex(rules["0"], rules)
    for m in messages:
...
if __name__ == '__main__':
    ...
    for rule in RULES.split("\n"):
        name, r = rule.split(": ")
        rules[name] = r
    print(execute_with_regex(messages, rules))
