...
def check_rules(rule, value):
    [rule_repeat, rule_letter] = rule.split(" ")
    [min, max] = rule_repeat.split("-")
    min = int(min) - 1
    max = int(max) - 1
    if not ((value[min] == rule_letter and value[max] != rule_letter) or (value[min] != rule_letter and value[max] == rule_letter)):
        return value
...
if __name__ == '__main__':
    count = 0
    for input in INPUT:
        [rule, value] = input.split(": ")
        if not check_rules(rule, value):
            count += 1
    print(count)